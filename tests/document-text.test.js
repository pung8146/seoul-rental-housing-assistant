...
describe('fetchDocumentTexts', () => {
    it('extracts text from flate-compressed PDF streams before falling back to strings', async () => {
        const result = await fetchDocumentTexts([{ title: '서울 청년 입주자 모집공고문.pdf', url: 'https://example.com/notice.pdf' }], async () => new Response(toArrayBuffer(makePdf('신청자격 만 19세 이상 만 39세 이하 무주택자 월평균소득 3,589,957원 이하 총자산 34,500만원 이하'))));
    ...
    it('extracts text from HWPX zip XML documents', async () => {
        const result = await fetchDocumentTexts([{ title: '서울 청년 입주자 모집공고문.hwpx', url: 'https://example.com/notice.hwpx' }], async () => new Response(toArrayBuffer(makeZipXml('Contents/section0.xml', '<root><p>신청자격 만 19세 이상 만 39세 이하 무주택자 월평균소득 3,589,957원 이하 총자산 34,500만원 이하</p></root>'))));
        expect(result.failures).toEqual([]);
        expect(result.results).toMatchObject([
            {
                method: 'zip-xml',
