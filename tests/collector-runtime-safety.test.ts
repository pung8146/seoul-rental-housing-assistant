...
function createFixture(label = 'primary'): Fixture {
  ...
  writeExecutable(
    join(nodeBinDirectory, 'node'),
    '#!/usr/bin/env bash\n[ "${1:-}" = "-p" ] && printf "linux\\n"\n',
  );
  writeExecutable(
    join(nodeBinDirectory, 'npm'),
    `#!/usr/bin/env bash
...
`,
  ...
  writeExecutable(
    join(scriptsDirectory, 'publish-public-dashboard.sh'),
    `#!/usr/bin/env bash
...
function runCollector(fixture: Fixture, timeout = 2_000) {
...
test.each(['fifo', 'symlink'] as const)(
  'rejects a %s collect.flock promptly without starting work',
  (kind) => {
...
test('canonicalizes every runtime path against the invocation cwd before changing cwd', () => {
  ...
  for (const line of lines) {
    for (const value of expected) expect(line).toContain(value);
  }
...
test('keeps each invocation cwd lock and database in the same canonical scope', () => {
  ...
  expect(firstLog).toContain(
    `db=${join(first.stateDirectory, 'data/rental-housing.db')}`,
  );
  expect(secondLog).toContain(`state=${second.stateDirectory}`);
  expect(secondLog).toContain(
    `db=${join(second.stateDirectory, 'data/rental-housing.db')}`,
  );
