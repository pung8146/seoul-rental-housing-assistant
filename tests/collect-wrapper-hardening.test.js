...
function createFixture() {
    ...
    writeExecutable(join(scriptDirectory, 'publish-public-dashboard.sh'), `#!/usr/bin/env bash\nprintf 'publish:%s\\n' "$PWD" >> "$STUB_WORK_LOG"\n`);
    writeExecutable(join(toolchainDirectory, 'node'), `#!/usr/bin/env bash\nif [ "\${1:-}" = '-p' ]; then\n  printf 'linux\\n'\n  exit 0\nfi\nprintf 'node:%s\\n' "$PWD" >> "$STUB_WORK_LOG"\n`);
    writeExecutable(join(toolchainDirectory, 'npm'), `#!/usr/bin/env bash\nprintf 'collect:%s\\n' "$PWD" >> "$STUB_WORK_LOG"\n`);
    return {
        appDirectory,
        collectScript,
        environment: {
...
function runCollector(fixture, environment = fixture.environment) {
    return spawnSync('/usr/bin/bash', ['-c', 'umask 022; exec /usr/bin/bash "$1"', 'runner', fixture.collectScript], {
...
async function holdLock(fixture) {
    ...
    const holder = spawn('/usr/bin/bash', [
        '-c',
...
test('a relative node bin stays canonical after cd and exported functions cannot win', () => {
    ...
    const environment = {
        ...fixture.environment,
        'BASH_FUNC_node%%': `() { printf 'node-function\\n' >> "$STUB_FUNCTION_LOG"; printf 'linux\\n'; }`,
        'BASH_FUNC_npm%%': `() { printf 'npm-function\\n' >> "$STUB_FUNCTION_LOG"; return 86; }`,
