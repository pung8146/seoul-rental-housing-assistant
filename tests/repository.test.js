...
describe('sqlite repository', () => {
    it('uses all default collection adapters', () => {
        expect(createDefaultAdapters().map((adapter) => adapter.source)).toEqual([
            'lh',
            'sh',
            'gh',
            'soco',
    ...
    it('extracts eligibility requirements from primary attachment text when detail HTML is incomplete', async () => {
        ...
        await runCollect({
            adapters: [adapter],
            repository,
            documentFetch: async () => new Response('신청자격: 만 19세 이상 만 39세 이하 무주택자. 월평균소득 3,589,957원 이하, 총자산 34,500만원 이하, 자동차가액 3,708만원 이하'),
        });
