...
function installPausingGitWrapper(fixture) {
...
async function runPublisherAcrossGitPause(fixture, feed, pauseMatch, pauseOccurrence, mutate, overrides = {}) {
    ...
    const completed = new Promise((resolveCompletion) => {
...
describe('publisher repository safety', () => {
    ...
    test.each(['fetch', 'push'])('rejects multiple origin %s URLs before exporting or mutating', (urlKind) => {
        const fixture = createFixture();
        ...
        const feedBefore = readFileSync(join(fixture.dashboardDirectory, 'public', 'public-feed.json'), 'utf8');
        const result = runPublisher(fixture, '{"notices":[{"id":"new"}]}\n', {
        ...
        expect(readFileSync(join(fixture.dashboardDirectory, 'public', 'public-feed.json'), 'utf8')).toBe(feedBefore);
    });
    test('does not force-update from a stale local HEAD captured before the baseline', async () => {
        ...
        const result = await runPublisherAcrossGitPause(fixture, '{"notices":[{"id":"new"}]}\n', 'rev-parse refs/remotes/origin/main', 1, () => {
    ...
    test('does not resurrect a pending commit after an operator resets main', async () => {
        const fixture = createFixture();
        const remoteBefore = git(fixture.remoteDirectory, 'rev-parse', 'main');
        writeFileSync(join(fixture.dashboardDirectory, 'public', 'public-feed.json'), '{"notices":[{"id":"pending"}]}\n');
        git(fixture.dashboardDirectory, 'add', 'public/public-feed.json');
        git(fixture.dashboardDirectory, 'commit', '-m', 'data: update public housing notices');
        const pendingHead = git(fixture.dashboardDirectory, 'rev-parse', 'HEAD');
        writePendingMarker(fixture, pendingHead);
        const result = await runPublisherAcrossGitPause(fixture, '{"notices":[{"id":"pending"}]}\n', 'remote get-url --push', 2, () => {
