...
describe('collect and notify process outcome', () => {
  it('marks the process failed when no collection source succeeds', async () => {
    ...
    const markCollectionProcessOutcome = (
      notifyModule as unknown as {
        markCollectionProcessOutcome: (successfulSourceCount: number) => void;
      }
    ).markCollectionProcessOutcome;
