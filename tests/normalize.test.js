...
describe('core domain schemas', () => {
    ...
    it('rejects a notice missing required identifying fields', () => {
        ...
        if (!result.success) {
            expect(result.error.issues.map((issue) => issue.path.join('.'))).toEqual(expect.arrayContaining(['source', 'title', 'stableKey', 'changeHash']));
        }
    });
...
describe('adapter contract', () => {
    ...
    it('does not fall back to LH fixture notices when live parsing returns no rows', async () => {
        const adapter = createLhAdapter({
            fetch: async () => new Response('<table><tbody><tr><td>검색 결과가 없습니다.</td></tr></tbody></table>', {
                status: 200,
                headers: { 'Content-Type': 'text/html; charset=utf-8' },
            }),
        });
...
describe('normalization helpers', () => {
    ...
    it('keeps notice stableKey for identity while changeHash tracks meaningful notice content changes', () => {
        const makeNotice = (overrides = {}) => normalizeAdapterOutput({
            source: 'lh',
            notices: [
                {
                    sourceId: 'notice-2',
