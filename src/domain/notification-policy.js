...
export const getNotificationPriority = (event, profile) => {
...
const isActionableEvent = (event, profile) => getNotificationPriority(event, profile) !== 'low';
export const filterNotificationEvents = ({ events, profile, policy = 'actionable', }) => {
    if (policy === 'all') {
        return events;
    }
    return events.filter((event) => isActionableEvent(event, profile));
};
export const groupNotificationEvents = ({ events, profile, policy = 'actionable', }) => {
    const groups = {
        high: [],
        review: [],
        low: [],
...
export const shouldNotifyCollectResult = ({ events, failures, profile, policy = 'actionable', }) => failures.length > 0 || filterNotificationEvents({ events, failures, profile, policy }).length > 0;
