...
const isSaleNotice = (notice) => inferNoticeTypeLabels(notice).includes('분양');
...
const hasEligibilityRequirements = (notice) => {
...
const applicationPeriodQuality = (notice) => {
...
const applicationAttachmentQuality = (notice, attachments) => {
...
const renderDetailQuality = (notice, listings, attachments) => {
    const items = [
...
const preparationStatusLabel = (status) => {
    if (status === 'ready') {
        return '준비됨';
    }
    if (status === 'review') {
...
const renderPreparationChecklist = (notice, listings, attachments) => {
    const items = [
        {
            label: '신청 링크',
