...
type SeoulHousingFetch = typeof fetch;

export type CreateSeoulHousingAdapterOptions = {
  fetch?: SeoulHousingFetch;
};
...
const extractHref = (rowHtml: string): string | undefined => {
...
const extractSeq = (sourceUrl: string | undefined, fallback: string): string => {
  if (!sourceUrl) {
    return fallback;
  }
...
export const parseSeoulHousingPublicLeaseHtml = (html: string): RawNoticeCandidate[] => {
...
export const createSeoulHousingAdapter = (options: CreateSeoulHousingAdapterOptions = {}): SourceAdapter => {
