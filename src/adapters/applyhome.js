...
const normalizeTitle = (name) => /공고|모집/.test(name) ? name : `${name} 입주자모집공고`;
...
const getItems = (payload) => {
    ...
    if (Array.isArray(root.data)) {
        return root.data.filter((item) => typeof item === 'object' && item !== null);
    }
    const item = root.response?.body
        ?.items;
    const rawItems = item?.item;
    if (Array.isArray(rawItems)) {
        return rawItems.filter((entry) => typeof entry === 'object' && entry !== null);
    }
...
const rowToNotice = (row) => {
    ...
    const applicationStartAt = normalizeDate(firstString(row, ['SPSPLY_RCEPT_BGNDE', 'GNRL_RCEPT_BGNDE', 'spsplyRceptBgnde', 'gnrlRceptBgnde']));
    const applicationEndAt = normalizeDate(firstString(row, ['GNRL_RCEPT_ENDDE', 'SPSPLY_RCEPT_ENDDE', 'gnrlRceptEndde', 'spsplyRceptEndde']));
...
export const parseApplyHomeNoticeResponse = (body) => {
    const parsed = JSON.parse(body);
    return getItems(parsed)
        .map(rowToNotice)
        .filter((notice) => notice !== null);
};
const buildApiUrl = (serviceKey) => {
...
export const createApplyHomeAdapter = (options = {}) => {
