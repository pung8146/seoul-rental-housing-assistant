...
type SocoFetch = typeof fetch;

type SocoNoticeJsonItem = {
...
type SocoNoticeJsonResponse = {
  resultList?: SocoNoticeJsonItem[];
};

export type CreateSocoAdapterOptions = {
  fetch?: SocoFetch;
};
...
const extractCells = (rowHtml: string): string[] =>
  Array.from(rowHtml.matchAll(/<td\b[^>]*>([\s\S]*?)<\/td>/gi)).map((match) => stripHtml(match[1]));
...
const extractDates = (value: string): string[] =>
  Array.from(value.matchAll(/(\d{4})\s*(?:년|[-.])\s*(\d{1,2})\s*(?:월|[-.])\s*(\d{1,2})\s*일?/g)).map(
    (match) => `${match[1]}-${match[2].padStart(2, '0')}-${match[3].padStart(2, '0')}`,
  );

const decodeHtmlEntities = (value: string): string =>
  value.replace(/&amp;/gi, '&').replace(/&#40;/g, '(').replace(/&#41;/g, ')');

const toAbsoluteSocoUrl = (value: string): string => new URL(decodeHtmlEntities(value), SOCO_ORIGIN).toString();

const extractTitleAndUrl = (rowHtml: string): { title: string; sourceUrl: string; sourceId: string } | null => {
...
const inferCategory = (cells: string[]): string | undefined =>
  cells.find((cell) => /^(최초|추가|민간|공공)$/.test(cell));

const mapRentalCategory = (value: string | undefined): string | undefined => {
  if (value === '1') {
    return '공공';
  }
  if (value === '2') {
...
const mapRoundCategory = (value: string | undefined): string | undefined => {
  if (value === '1') {
    return '최초';
  }
  if (value === '2') {
...
const buildTags = (category: string | undefined): string[] => {
...
const buildSocoNotice = ({
  ...
  const targetTags = ['청년', '청년안심주택', ...[category, round].filter((value): value is string => value != null)];
...
export const parseSocoNoticeListHtml = (html: string): RawNoticeCandidate[] => {
  ...
  for (const [, rowHtml] of rows) {
    ...
    const postedAt = cells.map(normalizeDate).find((date): date is string => date != null);
...
export const parseSocoNoticeListJson = (payload: SocoNoticeJsonResponse): RawNoticeCandidate[] =>
  (payload.resultList ?? [])
    .map((item) => {
      ...
      const sourceUrl = new URL(
        `view.do?optn1=${encodeURIComponent(item.optn1 ?? '')}&boardId=${encodeURIComponent(sourceId)}&menuNo=400008&pageIndex=1`,
        SOCO_NOTICE_LIST_URL,
      ).toString();
...
export const createSocoAdapter = (options: CreateSocoAdapterOptions = {}): SourceAdapter => {
  ...
  return {
    ...
    async fetchNotices() {
      ...
      const payload = (await response.json()) as SocoNoticeJsonResponse;
