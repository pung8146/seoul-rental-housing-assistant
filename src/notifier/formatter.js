...
const formatApplicationPeriod = (notice) => {
...
const formatDetailQuality = (notice, listings) => {
...
const eventNoticeKey = (event) => `${event.notice.source}:${event.notice.sourceId}:${event.type === 'new_notice' ? event.occurredAt : 'listing'}`;
const summarizeEventGroup = (events) => {
...
const summarizeFailures = (failures) => {
    const detailFailureCounts = new Map();
    const lines = [];
